...
def get_corr_coeff(pred_imp_scores, video, dataset):
    ...
    * Applicable only for the TVSum dataset.
    ...
    annot_path = f"../data/{dataset}/ydata-anno.tsv"
    ...
    for annot in range(len(user_scores)):
        ...
        curr_rho_coeff, _ = spearmanr(pred_imp_scores, true_user_score[:-1])
        curr_tau_coeff, _ = kendalltau(rankdata(pred_imp_scores), rankdata(true_user_score[:-1]))
